...
/*   Updated: 2022/10/19 23:26:49 by jtaravel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

int	check_next_pos_d_2(t_g *g, double angle)
{
...
int	angle_check_d(t_g *g, double angle)
{
	if (angle > 90 * PI / 180 && angle <= 180 * PI * 180)
	{
		if (g->m.map[(int)(g->c.y + (sin(angle) * (WD_SPEED))) / SIZE + 1]
			[(int)(g->c.x + (cos(angle) * (WD_SPEED))) / SIZE] == '1'
			&& (g->m.map[(int)(g->c.y + (sin(angle) * (WD_SPEED))) / SIZE]
...
{
	if ((int)(g->c.y + (sin(angle) * (WD_SPEED))) / SIZE != (int)g->c.y / SIZE
		...
		!= (int)g->c.x / SIZE)
	{
		if (angle >= 0 * PI / 180 && angle <= 90 * PI / 180)
		{
			...
			if (!angle_check_d(g, angle))
				return (0);
	}
	if (!check_next_pos_d_2(g, angle))
		return (0);
	return (1);
}
